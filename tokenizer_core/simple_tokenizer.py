...
def default_allowed_boundaries(text: str, lang: str | None = None, device: torch.device | None = None):
    clusters = grapheme_clusters(text)
    B = torch.zeros(len(text)+1, dtype=torch.bool, device=device or default_device())
    B[0] = True; pos = 0
    for g in clusters:
        pos += len(g)
        B[pos] = True
...
class ScalableTokenizer:
    def __init__(self, **kwargs):
        ...
        self.device = torch.device(kwargs.get("device")) if kwargs.get("device") else default_device()
    ...
    def _dp_costs(self, para_idx):
        ...
        dp = torch.full((T + 1, K), float('inf'), dtype=torch.float32, device=self.device)
        dp[0, idx["<BOS>"]] = 0.0
        ...
        for t in range(1, T + 1):
            for L in range(1, min(self.max_token_len, t) + 1):
                ...
                if not math.isfinite(base_cost): continue
                ...
                for si in range(K):
                    ...
                    cost = prev_cost + base_cost + add_cost
                    current = float(dp[t, j].item())
                    if cost < current:
                        dp[t, j] = cost
        return dp.detach().to("cpu")
    ...
    def _find_best_new_tokens_batch(self, all_dp_costs, top_k):
        ...
        for tok in self._potential_tokens:
            ...
            for para_idx, start_pos in self._token_occurrences.get(tok, []):
                ...
                baseline_cost = float(torch.min(dp[end_pos, :]).item())
                if not math.isfinite(baseline_cost): continue
                
                best_prev_cost = float('inf')
                for si in range(len(self._classes)):
                    add_cost = self._ling.additive_cost(tok, self._classes[si], para_idx)
                    prev_val = float(dp[start_pos, si].item())
                    cost = prev_val + proposer_cost + add_cost # Use proposer cost here
    ...
    def tokenize(self, text, lang=None):
        ...
        toks, t, c_idx = [], T, int(torch.argmin(dp[T, :]).item())
        while t > 0:
