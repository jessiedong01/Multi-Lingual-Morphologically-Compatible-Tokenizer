...
class ScalableTokenizer:
    ...
    def __init__(self, max_token_len=14, min_freq=5, alpha=1.0, beta=0.5, tau=0.01,
                 ...
                 prefix_reward=0.025, suffix_reward=0.01):
        ...
        Args:
            ...
            prefix_reward (float): Reward applied when tokens begin with known prefixes.
            suffix_reward (float): Reward applied when tokens end with known suffixes.
        """
        # --- Hyperparameters ---
    ...
    def set_feature_models(self, **kwargs):
        ...
        if "space_penalty" not in kwargs:
            kwargs["space_penalty"] = self.space_penalty
        if "prefix_reward" not in kwargs:
            kwargs["prefix_reward"] = self.prefix_reward
        if "suffix_reward" not in kwargs:
            kwargs["suffix_reward"] = self.suffix_reward
        self._ling = LinguisticModels(morphology_kwargs=morph_kwargs, **kwargs)
        # Keep tokenizer-level defaults in sync with the configured models.
        self.space_penalty = self._ling.space_penalty
        self.prefix_reward = self._ling.prefix_reward
        self.suffix_reward = self._ling.suffix_reward
    ...
    def save(self, path, include_morphology=True):
        ...
        payload = {
            "config": {
                ...
                "space_penalty": self.space_penalty,
                "prefix_reward": self.prefix_reward,
                "suffix_reward": self.suffix_reward,
                "linguistic_kwargs": {
    ...
    def load_from_file(cls, path):
        ...
        tok = cls(
            ...
            space_penalty=cfg.get("space_penalty", ling_kwargs.get("space_penalty", 0.1)),
            prefix_reward=cfg.get("prefix_reward", ling_kwargs.get("prefix_reward", 0.025)),
            suffix_reward=cfg.get("suffix_reward", ling_kwargs.get("suffix_reward", 0.01)),
        )
        token_bigram = {}
        ...
        if morph_payload:
            ...
            morph.token_vec = {k: np.array(v, dtype=np.float32) for k, v in morph_payload.get("token_vec", {}).items()}
            morph.lang_proto = {k: np.array(v, dtype=np.float32) for k, v in morph_payload.get("lang_proto", {}).items()}
    ...
    def _dp_decode(self, paragraph_idx: int, restrict_vocab=None, decode_only=True):
        ...
        for t in range(1, T + 1):
            ...
            for L in range(1, maxL + 1):
                ...
                for si in range(K):
                    prev_cost = dp[i, si]
                    if not np.isfinite(prev_cost): continue
        ...
        end_cls = int(np.argmin(dp[T]))
        dp_min = np.minimum.reduce(dp, axis=1)

        def backtrace():
            """Reconstructs the token sequence from the backpointers."""
            if not np.isfinite(dp[T, end_cls]):
    ...
    def _find_best_new_tokens_batch(self, all_dp_min, top_k=8, max_base_cost=50.0):
        ...
        Args:
            all_dp_min (list[np.array]): A list of the minimum DP costs for each paragraph.
        ...
        for tok in self._potential_tokens:
            ...
            for (pi, start) in self._token_occurrences.get(tok, []):
                ...
                if not (np.isfinite(proposal_cost) and np.isfinite(dpmin[start]) and np.isfinite(dpmin[end])): continue
                # Reduced Cost = (cost of new path) - (cost of old path)
                # rc = (cost of `tok`) + (cost to reach `start`) - (cost to reach `end`)
                rc = proposal_cost + dpmin[start] - dpmin[end]
